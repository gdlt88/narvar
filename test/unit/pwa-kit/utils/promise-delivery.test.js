...
describe('PWA-Kit Promise Delivery Utility', function () {
    ...
    describe('getTransitDaysForShippingMethod', function () {
        it('should return 1 day for overnight shipping regardless of ZIP', function () {
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('overnight', '10001'), 1)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('overnight', '90210'), 1)
        })

        it('should return max 2 days for express shipping', function () {
            // For close ZIPs (1 day base), express should be 1 day
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('express', '10001'), 1)

            // For far ZIPs (5 day base), express should be capped at 2 days
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('express', '90210'), 2)
        })

        it('should return base transit days for standard shipping', function () {
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('standard', '10001'), 1)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('standard', '30301'), 2)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('standard', '50000'), 3)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('standard', '60601'), 4)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('standard', '90210'), 5)
        })

        it('should default to standard shipping for unknown methods', function () {
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod('unknown', '90210'), 5)
            assert.equal(promiseDelivery.getTransitDaysForShippingMethod(null, '90210'), 5)
        })
    })
    ...
    describe('CONFIG', function () {
        it('should have CUTOFF_HOUR_EST set to 14 (2 PM)', function () {
            assert.equal(promiseDelivery.CONFIG.CUTOFF_HOUR_EST, 14)
        })

        it('should have ORIGIN_ZIP set to NYC', function () {
            assert.equal(promiseDelivery.CONFIG.ORIGIN_ZIP, '10001')
        })

        it('should have EST_OFFSET set to -5', function () {
            assert.equal(promiseDelivery.CONFIG.EST_OFFSET, -5)
        })
    })
})
